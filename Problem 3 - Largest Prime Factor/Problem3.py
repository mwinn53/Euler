...
## ===========================================================================
#
# APPROACH 1: Sieve of Eratosthenes	(SieveOfEratosthenes)
#   [*** NOT FEASIBLE FOR N>10^8 ***]
#   - Build table of Prime numbers from (2..N)
#
#       An optimized implementation (starting from squares), in pseudocode,
#       finding all the primes not greater than n:
#       SOURCES:    http://en.wikipedia.org/wiki/Sieve_of_Eratosthenes
...
#       ---------------------------------------------------------------
#
#   - Use trial division from largest n
#
# APPROACH 2: Pollard's RHO Method (PollardRHO)
#
#       ---------------------------------------------------------------
#       x=2; y=2; d=1;
#       While d == 1:
#           x = g(x)             g(x) = x^2 - 1 mod n, or more commonly
#           y = g(g(y))          g(x) = x^2 + 1 mod n.
#           d = gcd(|x - y|, n)
#       If d = n, return failure.
#       Else, return d.
#       ---------------------------------------------------------------
#
# OTHER APPROACHES: Lenstra elliptic curve factorization (<10^{25})
...
# ---------------------------------------------------------------------------
# function: PollardRHO()
#   description:    Calculate a series of prime numbers from 2 to N
#   input:  	    N
#   output:         list of prime numbers from 2 to N
# ---------------------------------------------------------------------------
def PollardRHO(n):
    '''Uses Pollard's RHO method of factorization'''
...
def main():
    ...
    s = SieveOfEratosthenes1(number)
#    print s
    print 'Number of primes in s:\t{0}'.format(len(s))
    print '----------------------------------------------------------------------\n'

    s = list(SieveOfEratosthenes2(number))
    print 'Number of primes in s:\t{0}'.format(len(s))
    print '----------------------------------------------------------------------\n'
#    print s

    s = list(SieveOfEratosthenes3(number))
    print 'Number of primes in s:\t{0}'.format(len(s))
    print '----------------------------------------------------------------------\n'
#    print s
    #pass
